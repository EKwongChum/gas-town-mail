...
```bash
java -jar journal-archiver/target/journal-archiver-1.2.0.jar
...
```bash
java -jar mail-cleaner/target/mail-cleaner-1.2.0.jar
```
...
- **mail-cleaner（29）**：消费成功/失败重投、批量消息部分失败、消息 traceId 恢复与清理、
  S3 读取 + 邮件解析 + 写入
