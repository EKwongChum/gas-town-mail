...
- **mail-mcp-server（147）**：ES 查询服务（分页搜索、页大小上限、计数、按 id 查询）、
  ...
  投递可靠性另有用例：`Idempotency-Key` 去重（重复请求只投递一次、投递中返回 409、失败释放 key、
  TTL 过期、空白/超长 key 校验）、异步投递（202 + 任务查询、任务生命周期、队列满 429、禁用时 400、
  真实容器下 multipart + 异步的附件物化），以及真实隐式 TLS 握手（内嵌 TLS SMTP 服务器 +
  自签测试证书：证书匹配放行、主机名不匹配被拒、关闭校验后放行）。
