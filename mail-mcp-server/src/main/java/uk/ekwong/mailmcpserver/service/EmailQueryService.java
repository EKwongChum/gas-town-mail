...
/**
 * Reads the archived email metadata from the Elasticsearch {@code mail_info} index. Used by the MCP
 * tools.
 */
@Service
public class EmailQueryService {
    ...
    /**
     * Searches the {@code mail_info} index with the given filters, returning the requested page
    ...
    public long count(MailSearchRequest request) {
        NativeQuery query = NativeQuery.builder().withQuery(buildQuery(request)).build();
        return operations.count(query, MailInfoDocument.class);
    }

    /**
     * Returns the document with the given archive id (MongoDB id / object storage key), or empty
    ...
    /**
     * Builds a bool query: exact term filters (AND) plus, when a keyword is given, a
     * minimum-should-match clause searching subject, sender, from, to and message id.
     */
                                            ...
                                            r ->
                                                    r.date(
                                                            d -> {
                                                                ...
                                                                if (request.receivedTimeGe()
                                                                        != null) {
                                                                    d.gte(
                                                                            request.receivedTimeGe()
                                                                                    .toString());
                                                                }
                                                                if (request.receivedTimeLt()
                                                                        != null) {
                                                                    d.lt(
                                                                            request.receivedTimeLt()
                                                                                    .toString());
                                                                }
                                                                return d;
                                                            }))));
